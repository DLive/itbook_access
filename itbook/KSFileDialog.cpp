	...
	BOOL bOpenFileDialog, CDocTemplate* pTemplate)
{
	...
	m_ofn.Flags&=OFN_EXPLORER|OFN_PATHMUSTEXIST|OFN_OVERWRITEPROMPT|OFN_HIDEREADONLY;//|OFN_ENABLEHOOK;
	int nResult = DoModal();
...
BOOL CKSFileDialog::DoSave(LPCTSTR lpszPathName, BOOL bReplace)
{
	...
	if (newName.IsEmpty())
	{
		...
		if (!(DoPromptFileName(newName,bReplace ? AFX_IDS_SAVEFILE : AFX_IDS_SAVEFILECOPY,
		  OFN_PATHMUSTEXIST|OFN_HIDEREADONLY, FALSE, pTemplate)))
			return FALSE;       // don't even attempt to save
	}

	//test
	//CMainFrame   *pMain=(CMaimFrame   *)AfxGetApp()-> m_pMainWnd; 
	//CitbookView   *pView=(CyouView   *)pMain-> GetActiveView();
	//CInputDB indb(m_pDoc);
	//indb.getText(newName);
	//endtest
